...
export function renderLegacyAuditReport(report: LegacyAuditReport): string {
  ...
  lines.push("");
  lines.push("## H12 — Turnaround watchlist criteria (§7)");
  lines.push("");
  lines.push("**Hypothesis:** " + report.verdicts.h12.hypothesis);
  lines.push("");
  lines.push(`**Verdict:** **${report.verdicts.h12.verdict}** — ${report.verdicts.h12.evidence}`);
  lines.push("");
  lines.push("| Cohort | Horizon | N | Mean excess | CI (95%) |");
  lines.push("|---|---|---|---|---|");
  for (const r of report.turnaroundRows) {
    const ex =
      r.meanForwardExcess === null
        ? "—"
        : `${(r.meanForwardExcess * 100).toFixed(2)}%`;
    const ci = r.excessCi95
      ? `[${(r.excessCi95.lo * 100).toFixed(2)}%, ${(r.excessCi95.hi * 100).toFixed(2)}%]`
      : "—";
    lines.push(`| ${r.cohort} | ${r.horizon}y | ${r.nObservations} | ${ex} | ${ci} |`);
  }
  lines.push("");
