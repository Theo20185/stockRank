...
| **DelistedRecovery-2026-04-25** | Phase 2D: how many of the 345 delisted S&P 500 symbols can we fetch EDGAR + chart data for? | --include-delisted, PIT, all symbols from Wikipedia changes table | **infrastructure-built / recovery 0% on EDGAR** | chart 143/345 (41.4%); EDGAR 0/345 (0.0%); both 0/345. Local CIK lookup excludes delisted tickers — even still-trading ones removed for cap change. | `docs/backtest-actions-2026-04-25-phase2.md` §2 | Document gap; queue Phase 2D.1 (CIK lookup fallback to SEC's broader table) as next priority | After CIK lookup expansion |
