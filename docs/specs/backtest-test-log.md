...
| **LongShort-default-PIT-2018to2023** | Phase 4A: where does value-deep's edge come from — top picks or bottom avoidance? | --weight-test, PIT+delisted | **edge is mostly tail-avoidance** | top 3y -2.03%, bottom 3y -25.01%, L/S delta +22.98 pp. The composite barely beats SPY at the top; the edge is in NOT picking the bottom. | `docs/backtest-weight-validation-2026-04-26.md` | None — informative only | 12-18 months |
| **LongShort-default-PIT-2010to2018** | Same, pre-COVID | --weight-test, PIT+delisted, pre-COVID | **signal INVERTS** | top 3y -0.86%, bottom 3y +5.03%, L/S delta -5.88 pp. Bottom decile actually outperforms top in 2016-2019 forward windows. | `docs/backtest-weight-validation-2026-04-26-precovid.md` | None — informative; explains why composite is "least bad" not "actually winning" | Same |
| **RiskAdjusted-PIT-2018to2023** | Phase 4B: does any candidate flip from reject to adopt under Sharpe/Sortino instead of mean? | --weight-test, PIT+delisted | **no flip** | value-deep also leads on Sharpe-like (-0.32) and Sortino-like (-0.31) at 3y; everything else is worse on both metrics. Max DD smallest for value-deep at -4.62% (vs -10.50% for legacy). | same | None | 12-18 months |
| **RiskAdjusted-PIT-2010to2018** | Same, pre-COVID | --weight-test, PIT+delisted, pre-COVID | **no flip** | same pattern; momentum-on +2.47% the only positive but CI crosses zero | same | None | Same |
| **H10-PIT-2018to2023** | Phase 4C: do declining-fvTrend names underperform stable+improving? | --fv-trend-audit, PIT+delisted | **fail (declining OUTPERFORMS)** | declining cohort 3y -4.76% vs stable+improving -10.05%; gap +5.30 pp in WRONG direction. Demotion rule actively harmful in COVID era. | `docs/backtest-fv-trend-audit-2026-04-26.md` | **fvTrend === "declining" bucket demotion REMOVED** in `buckets.ts` (this commit) | 12-18 months |
| **H10-PIT-2010to2018** | Same, pre-COVID | --fv-trend-audit, PIT+delisted, pre-COVID | **inconclusive** | declining vs stable+improving within 2 pp; classification counts: declining 11.9k, stable 2.7k, improving 19.5k | `docs/backtest-fv-trend-audit-2026-04-26-precovid.md` | Confirms removal — no clear edge in pre-COVID either | Same |
...
| 2026-04-26 (Phase 4 long/short + risk-adj + H10) | both PIT regimes + delisted | --weight-test --fv-trend-audit | 50 | engine extensions: bottom-decile, Sharpe/Sortino/MDD, fv-trend-audit module | `bcd5161` |
